...
- **vite-plugin-pwa** — PWA manifest + Workbox service worker (precaches build assets, caches channel logos via CacheFirst)
...
public/
  icon.svg            # App icon (SVG, used for favicon, manifest, and apple-touch-icon)
vite.config.ts        # Vite config with Tailwind, React, and VitePWA plugins
...
- **Manifest** — generated by `vite-plugin-pwa` at build time (`dist/manifest.webmanifest`)
- **Service worker** — Workbox `generateSW` strategy; precaches all JS/CSS/HTML/SVG build assets
- **Icon** — `public/icon.svg` (emerald TV icon on dark background); referenced as favicon, apple-touch-icon, and in the manifest
- **Logo caching** — channel logos from `i.imgur.com` are cached with a `CacheFirst` strategy (7-day TTL, max 500 entries)
- **Note** — iOS Safari does not support SVG `apple-touch-icon`. Add a `public/apple-touch-icon.png` (180×180) for full iOS home-screen support.
